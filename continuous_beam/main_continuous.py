...
def calculate_collection_efficiency(all_parameters,SHOW_PLOT,track_LET,track_radius):
    ...
    for entry,electric_field_entry in enumerate(electric_field_list):
        ...
        all_results = distribute_and_evolve(track_radius,track_LET, parameter_list, SHOW_PLOT,extended_time,fluence_rate)
...
if __name__ == "__main__":
    ...
    unit_length = 4e-4      # cm, size of every voxel length
    SHOW_PLOT = False

    LET_keV_um = 7.760e-4  # linear energy transfer [keV/micrometer]
    LET_eV_cm = LET_keV_um * 1e7

    track_radius = 0.001  #cm, from Kanai (1998)
    track_LET = LET_eV_cm

    extended_time = 10.
    if len(sys.argv) > 1:
        fluence_rate = float(sys.argv[1])
    else:
        raise ValueError("Fluence-rate is not given as input")

    params = [b_cm, d_cm, theta, electric_field_list, unit_length, W, ion_mobility,ion_diff,alpha]
    f_IonTracks = calculate_collection_efficiency(params,SHOW_PLOT,track_LET,track_radius)

    print("# f_IonTracks\tfluence-rate")
    for i,f in enumerate(f_IonTracks):
        text = "  %g\t%g" % (f,fluence_rate)
        print(text)
