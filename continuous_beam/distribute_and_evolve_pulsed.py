...
@cython.locals( # variables
                ...
                SHOW_PLOT='bint',
                fluence_rate='double',
                extended_time='double',
                #locals
                )
def distribute_and_evolve(track_radius,track_LET,parameter_list,SHOW_PLOT,extended_time,fluence_rate):
    return f(track_radius,track_LET,parameter_list,SHOW_PLOT,extended_time,fluence_rate)
...
@cython.header( # variables
                ...
                no_x='size_t',no_y='size_t',no_z='size_t',no_z_with_buffer='size_t',current_time='double',
                number_of_initialized_tracks='double',coordinate_counter='size_t',
                # indices and lengths
...
def f(track_radius,track_LET,parameter_list,SHOW_PLOT,extended_time,fluence_rate):
    ...
    computation_time_steps=int(d/(2.*ion_mobility*Efield*dt)*extended_time)

    number_of_iterations = computation_time_steps
    x_coordinates_ALL = rnd.uniform(0, 1, number_of_iterations * 50) * no_x
    y_coordinates_ALL = rnd.uniform(0, 1, number_of_iterations * 50) * no_y
    ...
    # one time step at a time, start the simulation
    for time_step in range(computation_time_steps):

        current_time = time_step*dt # actual simulation time at the pre

        # check that the number of initialized tracks matches the fluence-rate
        while number_of_initialized_tracks < fluence_rate*area*current_time:
