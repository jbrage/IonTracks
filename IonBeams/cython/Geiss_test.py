...
def Geiss_RRD_m(r_cm, c, a0_cm, r_max_cm):
...
def total_recombination_PDEsolver(list parameter_list):
    '''
    ...
    cdef double fluence_cm2_s   = parameter_list[0] # fluence-rate [/cm^2/s]
    cdef double LET_keV_um      = parameter_list[1] # linear energy transfer
    cdef double track_radius    = parameter_list[2] # Gaussian radius b [cm]
    cdef double voltage_V       = parameter_list[3] # [V/cm] magnitude of the electric field
    cdef double d_cm            = parameter_list[4] # [cm] # electrode gap
    cdef bint SHOW_PLOT         = parameter_list[5] # show frames of the simulation
    cdef int myseed             = parameter_list[6] # ensure the coordinates are sampled anew each run
    rnd.seed(myseed)                                # set the new seed
    cdef bint PRINT             = parameter_list[7] # print parameters?



    cdef double rho_material = 1.2e-3 # g/cm3
    cdef double density_ratio = 1.0 / rho_material
    a0_cm = 1e-7 * density_ratio
    # DEFINE E_MeV_u
    r_max_m = Geiss_r_max(E_MeV_u, rho_material)    
    
    cdef double c = LET_eV_cm / (np.pi * W) * (1 / (1 + 2 * np.log(r_max_cm / a0_cm)))
            ...
            for k in range(no_z_electrode, no_z + no_z_electrode):
                for i in range(no_xy):
                    for j in range(no_xy):
                        ...
                        ion_density = Geiss_RRD_m(distance_from_center, c, a0_cm, r_max_cm)
