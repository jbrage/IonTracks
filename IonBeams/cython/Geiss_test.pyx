...
def Geiss_PDEsolver(list parameter_list):
    ...
    cdef double voltage_V       = parameter_list[2] # [V/cm] magnitude of the electric field
    cdef double theta           = parameter_list[3] # [rad] angle between electric field and the ion track(s)
...
#    cdef double a0_cm = 1e-7 * density_ratio
    ...
    while not von_neumann_expression:
        ...
        cx = ion_mobility*Efield*dt/unit_length_cm*sin(theta)
        cy = 0
        cz = ion_mobility*Efield*dt/unit_length_cm*cos(theta)
    ...
    for k in range(no_z_electrode, no_z + no_z_electrode):
        for i in range(no_x):
            for j in range(no_x):
                distance_from_center = sqrt((i - mid_xy_array) ** 2 + (j - mid_xy_array) ** 2) * unit_length_cm
