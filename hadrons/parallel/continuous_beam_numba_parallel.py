...
def continuous_beam_PDEsolver(parameter_dic, extra_params_dic):
    ...
    stop = time()
    print(f"Avg time for inserting track step: {sum(times_insert_track_step)/len(times_insert_track_step)}s")
    print(f"Avg time for calculating new densities {sum(times_calculate_new_densities)/len(times_calculate_new_densities)}s")
