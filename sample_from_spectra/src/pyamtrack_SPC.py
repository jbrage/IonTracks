...
def extract_spectrum_at_range(spcdirpath, range_cmg):
    ...
    result = src.pyamtrack.AT_SPC_spectrum_at_range(spcdirpath, range_cmg, number_of_bins, depth_step, depth_g_cm2,
                                                    E_MeV_u, DE_MeV_u, particle_no, fluence_cm2)
...
def extract_range(filename):
    ...
    result = src.pyamtrack.AT_SPC_read_header_from_filename_fast(filename, E_MeV_u, peak_position_g_cm2, particle_no,
                                                                 material_no, normalisation, steps_no)
...
def scale_fluence_to_input_dose(input_dose_Gy, spectrum_dict):
    ...
    old_input_dose_Gy = src.pyamtrack.AT_total_D_Gy(len(E_MeV_u_input), E_MeV_u_input, particle_no_input,
                                                    fluence_cm2_input, material_no, stopping_power_source_no)
...
def get_spectrum_dictionary_step(spectrum):
    ...
    for step in depth_step_uniq:
        depth_step_new, depth_g_cm2_new, E_MeV_u_new, DE_MeV_u_new, particle_no_new, fluence_cm2_new = spectrum_at_depth_step(
            step, spectrum)
...
def dose_at_depth(depth_g_cm2, local_max, maximum, spectrum):
    ...
    dose_Gy = src.pyamtrack.AT_total_D_Gy(len(E_MeV_u_OK), E_MeV_u_OK, particle_no_OK, fluence_cm2_OK, material_no,
...
def survival(E_MeV_u, particle_no, fluence_cm2, m_number_of_targets, D0_characteristic_dose_Gy, sigma0_m2, kappa,
             er_model=2):
    non_zero_fluence_indexes = [i for i in range(len(fluence_cm2)) if fluence_cm2[i] > 0]
    E_MeV_u_OK = [E_MeV_u[i] for i in non_zero_fluence_indexes]
    particle_no_OK = [particle_no[i] for i in non_zero_fluence_indexes]
